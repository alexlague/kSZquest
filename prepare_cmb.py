...
mask_file = f"{paths.out_dir}/planck_galactic_mask_070.fits"
act_file = f"{paths.act_root}act_planck_dr5.01_s08s18_AA_{freq}_night_map_srcfree.fits"
ivar_file = f"{paths.act_root}act_planck_dr5.01_s08s18_AA_{freq}_night_ivar.fits"
...
enmap.write_map(f'mask_{freq}.fits',gmask)


ls,bells = utils.get_beam(freq)

def get_single_frequency_alms(imap, gmask,lmax,ls,bells,debug=False):
    imap[gmask<=0] = 0
    if debug:
